  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (err.response && err.response.status === 401) {
        setError(t('doctor_login.invalid'));
      } else {
        setError(t('doctor_login.failed') + err.message);
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div style={styles.wrapper}>
      <div style={styles.card}>
        <h2 style={{ marginTop: 0, color: colors.primary }}>{t('doctor_login.title')}</h2>
        <p style={{ color: colors.textMuted, fontSize: 13 }}>{t('doctor_login.subtitle')}</p>

        <Link to="/" style={styles.backLink}>{t('nav.back_home')}</Link>

        <form onSubmit={handleSubmit}>
          <div style={styles.field}>
            <label style={styles.label}>{t('doctor_login.email')}</label>
            <input type="email" value={email} onChange={e => setEmail(e.target.value)} required style={styles.input} />
          </div>

          <div style={styles.field}>
            <label style={styles.label}>{t('doctor_login.password')}</label>
          ...
          <button type="submit" disabled={loading} style={styles.button}>
            {loading ? t('doctor_login.submitting') : t('doctor_login.submit')}
          </button>
        </form>

        <p style={{ fontSize: 13, color: colors.textMuted, marginTop: 16, textAlign: 'center' }}>
          New doctor at your hospital? <Link to="/doctor/signup" style={{ color: colors.primary, fontWeight: 600 }}>Register here</Link>
        </p>
        <p style={{ fontSize: 12, color: colors.textLight, marginTop: 8, textAlign: 'center' }}>
          {t('doctor_login.demo')} <code style={styles.code}>mbarga@hgd.cm</code> / <code style={styles.code}>doctor123</code>
        </p>
      </div>
    </div>
  );
...
const styles = {
  ...
  button: { width: '100%', padding: '11px', background: colors.primary, color: '#fff', border: 'none', borderRadius: radius.md, fontSize: 14, fontWeight: 600, cursor: 'pointer', transition: transitions.base },
